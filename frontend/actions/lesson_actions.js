...
export const fetchAllLessonsByTeacher = teacherId => dispatch => {
  return APIUtil.fetchAllLessonsByTeacher(teacherId).then(lessons =>
    dispatch(receiveAllLessons(lessons))
  );
};

export const fetchLesson = id => dispatch => {
  return APIUtil.fetchLesson(id).then(lesson =>
    dispatch(receieveLesson(lesson))
  );
};
