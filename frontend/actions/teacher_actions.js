...
export const fetchAllTeachersByLanguage = languageName => dispatch => {
  return APIUtil.fetchAllTeachersByLanguage(languageName).then(teachers =>
    dispatch(receiveAllTeachers(teachers))
  );
};

export const fetchTeacher = id => dispatch => {
  return APIUtil.fetchTeacher(id).then(teacher =>
    dispatch(receiveTeacher(teacher))
  );
};
